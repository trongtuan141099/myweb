...
<body id="kt_app_body" data-kt-app-layout="dark-header" data-kt-app-header-fixed="true" data-kt-app-toolbar-enabled="true" data-kt-app-toolbar-fixed="true" class="app-default">
    ...
    <input type="hidden" id="returnUrl" value="/TangCa/DuyetTangCaKeHoach" />
    ...
    <input name="__RequestVerificationToken" type="hidden" value="AudoEIpNfxoj9nmC4kaxdXD1Q_1Gub0-8EMSCz2ud88afQp4R566SeupnARjSFPrjKYUAWB-FB79h6gYURDa5l1f8OqsadUAwLfFx3eb_hY1" />
        ...
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack gap-3">
            ...
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    <li class="breadcrumb-item text-muted">Duyệt tăng ca kế hoạch</li>
                </ul>
            </div>
            <!--end::Info-->
            <!--begin::Toolbar-->
    ...
    <script>
        ...
        var dataSource = new DevExpress.data.CustomStore({
            load: function (loadOptions) {
                ...
                $.getJSON("/Portal/TangCa/Get_DuyetTangCaKeHoach", params).done(function (result) {
                    isLoadingAll = false;
        ...
        $(document).ready(function () {
            ...
            $('#iThemMoi').click(function () {
                $('#iFormLabel_Update').text('Đăng ký nghỉ phép');
                NghiPhep = 0;
                Get_NoiDungSua();
                $('#mdDangKyNghiPhep').modal('show');
            });
        ...
        function ExportXLS() {
            loadPanel.show();
            $.post('/Portal/TangCa/ExportDuyetTangCaKeHoach', { TuNgay: TuNgay_S, DenNgay: DenNgay_S, __RequestVerificationToken: $('input[name="__RequestVerificationToken"]').val() }, function (r) {
                if (r.StatusCode > 0) {
                    downloadExcel(r.StatusText, 'Danh sách Duyệt tăng ca kế hoạch');
                } else {
        ...
        function LoadDanhSach() {
            $("#iDanhSach").dxDataGrid({
                ...
                columns: [
                    ...
                    { dataField: 'Nhom', caption: 'Nhóm', width: 120, alignment: "center" },
                    { dataField: 'ToDoi', caption: 'Tổ đội', width: 120, alignment: "center" },
                    { dataField: 'VaoCa', caption: 'Thời gian bắt đầu', width: 150, alignment: "center", dataType: "datetime", format : "dd/MM/yyyy HH:mm" },
                    { dataField: 'TanCa', caption: 'Thời gian kết thúc', width: 150, alignment: "center", dataType: "datetime", format: "dd/MM/yyyy HH:mm" },
                    { dataField: 'SoPhutOT', caption: 'Tổng số phút tăng ca', width: 170, alignment: "center" },
                    ...
                    {
                        dataField: 'isDuyet', caption: 'Trạng thái duyệt', width: 150, fixed: true, fixedPosition: "right", alignment: "center", cellTemplate: function (container, options) {
        ...
        <script type="text/javascript">
            ...
            function Duyet_TangCaKeHoach(TrangThai) {
                var tangca_ID = [];
                $.each($("#iDanhSach").dxDataGrid('instance').getSelectedRowsData(), function () {
                ...
                if (tangca_ID.join(',').length > 0) {
                    ...
                    $('#iDongY_Duyet').click(function () {
                        $.post('/Portal/TangCa/Update_DuyetTangCaKeHoach', { TangCa_ID: tangca_ID.join(','), DuyetGianTiep_ID: DuyetGianTiep_ID.join(','), IsDuyet_ID: IsDuyet_ID.join(','), LyDo: "", TrangThai: TrangThai, __RequestVerificationToken: $('input[name="__RequestVerificationToken"]').val() }, function (rs) {
                            if (rs.StatusCode == 1) {
                                DevExpress.ui.notify('Cập nhật thành công', "success", 3000);
                                LoadDanhSach();
            ...
            function TuChoi_TangCaKeHoach(TrangThai) {
                var tangca_ID = [];
                $.each($("#iDanhSach").dxDataGrid('instance').getSelectedRowsData(), function () {
                    tangca_ID.push(this.OT);
                })
                ...
                if (tangca_ID.join(',').length > 0) {
                    ...
                    var LyDo = $('#iLyDoTuChoi').val();
                    $('#iDongY_TuChoi').click(function () {
                        $.post('/Portal/TangCa/Update_DuyetTangCaKeHoach', { TangCa_ID: tangca_ID.join(','), DuyetGianTiep_ID: DuyetGianTiep_ID.join(','), IsDuyet_ID: IsDuyet_ID.join(','), LyDo: LyDo, TrangThai: TrangThai, __RequestVerificationToken: $('input[name="__RequestVerificationToken"]').val() }, function (rs) {
                            if (rs.StatusCode == 1) {
                                DevExpress.ui.notify('Cập nhật thành công', "success", 3000);
                                LoadDanhSach();
