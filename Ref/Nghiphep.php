...
<body id="kt_app_body" data-kt-app-layout="dark-header" data-kt-app-header-fixed="true" data-kt-app-toolbar-enabled="true" data-kt-app-toolbar-fixed="true" class="app-default">
    ...
    <input type="hidden" id="returnUrl" value="/NghiPhep/DuyetPhep" />
    ...
    <input name="__RequestVerificationToken" type="hidden" value="Vp_LMhfh8VMMUA0FyDaZK0eZRXpTEBYdGwO7oVJ-DbL8OgHVzCqJyOF3HZ4Lf2PgACASh4G0HLI2qsnCLDSil4efOY_6TvRcbeh1_SpQDCQ1" />
    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack gap-3">
            ...
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Duyệt ph&#233;p</h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    <li class="breadcrumb-item text-muted">Đơn xin ph&#233;p</li>
                </ul>
            </div>
            <!--end::Info-->
            <!--begin::Toolbar-->
...
<div class="card">
    <div class="card-header">
        ...
        <div class="card-toolbar bg-light-primary">
            <ul class="nav nav-tabs nav-bold nav-tabs-line">
                <li class="nav-item">
                    <label class="col-lg-12" style="margin-top: 10%;">Từ ng&#224;y:</label>
                </li>
                <li class="nav-item">
                    <div class="col-lg-12" id="id_TuNgay_S"></div>
                </li>
                <li class="nav-item">
                    <label class="col-lg-12" style="margin-top: 10%;">Đến ng&#224;y:</label>
                </li>
                <li class="nav-item">
                    <div class="col-lg-12" id="id_DenNgay_S"></div>
                </li>
                <li class="nav-item">
                    <label class="col-lg-12" style="margin-top: 10%;">Trạng th&#225;i:</label>
                </li>
                <li class="nav-item">
                    <div class="col-lg-12" id="id_TrangThai_S"></div>
                </li>
    ...
    <div class="card-body p-grid">
        <div id="iDanhSach" class="bsc-grid bsc-grid-header-center bsc-grid-hideheader" notitle="1">
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    <script>
        ...
        if ('vi' == 'jp')
            dataTrangThaiDuyet = [{ "ID": 0, "NAME": "全て" }, { "ID": 1, "NAME": "まだ承認されていません" }, { "ID": 2, "NAME": "承認済み" }, { "ID": 3, "NAME": "ごみ" }, { "ID": 4, "NAME": "許可申請の解除" }]
        else if ('vi' == 'en')
            dataTrangThaiDuyet = [{ "ID": 0, "NAME": "All" }, { "ID": 1, "NAME": "Refuse" }, { "ID": 2, "NAME": "Approved" }, { "ID": 3, "NAME": "Not approved yet" }, { "ID": 4, "NAME": "Cancellation of permission application" }]
        else
            dataTrangThaiDuyet = [{ "ID": 0, "NAME": "Tất cả" }, { "ID": 1, "NAME": "Chưa duyệt" }, { "ID": 2, "NAME": "Đã đồng ý duyệt" }, { "ID": 3, "NAME": "Đã từ chối duyệt" }, { "ID": 4, "NAME": "Nhân viên hủy đăng ký" }]
        ...
        let f_TrangThai = 0;
        var CapTrenTrucTiep = null, CapTrenGianTiep = null, HinhThucNghiPhep = null, LoaiNghiPhep = null, isNew = null, NghiPhep = null;
        var defaultLoadError = 'Đã có lỗi xảy ra, xin vui lòng thử lại.';
        var loginUrl = '/login';
        ...
        var dataSource = new DevExpress.data.CustomStore({
            load: function (loadOptions) {
                ...
                if (loadOptions.filter) {
                    ...
                    if (fil.indexOf('],"and",[') > 0) {
                        $.each(loadOptions.filter, function (i, v) {
                            if (i % 2 == 0) {
                                if (JSON.stringify(v).indexOf('],"and",[') > 0) {
                                    $.each(v, function (i2, v2) {
                                        if (i2 % 2 == 0) {
                                            filter.push({ Column: v2[0], Compare: v2[1], Value: v2[2] });
                                        }
                                    });
                                }
                                else {
                ...
                params.isLoadingAll = false;
                params.Take = loadOptions.take;
                params.Page = (loadOptions.skip / params.Take + 1) || 1;
                if (loadOptions.isLoadingAll || loadOptions.take == undefined) {
                    params.isLoadingAll = true;
                    params.Page = 1;
                    params.Take = 0;
                }
                ...
                $.getJSON("/Portal/NghiPhep/Get_DanhSachDuyetNghiPhep", params)
                    .done(function (result) {
                        if (result.StatusCode > 0) {
                            var rsData = JSON.parse(result.ResponseData);
                            if (rsData != null && rsData.length > 0) {
                                deferred.resolve(rsData, { totalCount: rsData[0].Total });
                                var dt = rsData[0];
                                $('#pnInfo').html(`<div class="notice d-flex bg-light-primary rounded border-primary border border-dashed  p-6">
                                    <div class="d-flex flex-stack flex-grow-1 ">
                                       <h4 class="text-gray-900 fw-bold">${dt.TieuDe}</h4>
                                    </div>
                                </div>`);
                                $('#pnInfo').removeClass('d-none');

                            }
                            else {
                                deferred.resolve([], { totalCount: 0 }); $('#pnInfo').addClass('d-none');
                            }
                        } else {
                            deferred.reject("Data Loading Error"); $('#pnInfo').addClass('d-none');
                        }
                    })
        ...
        function ExportXLS() {
            loadPanel.show();
            $.post('/Portal/NghiPhep/ExportDeDuyetNghiPhep', { TuNgay: TuNgay_S, DenNgay: DenNgay_S, __RequestVerificationToken: $('input[name="__RequestVerificationToken"]').val() }, function (r) {
                if (r.StatusCode > 0) {
                    downloadExcel(r.StatusText, 'Danh sách duyệt nghỉ phép');
                } else {
        ...
        function Get_NoiDungSua() {
            ...
            $.get('/Portal/Data/GetCapTrenTrucTiep').done(function (rs) {
                var data = JSON.parse(rs.ResponseData);
                CapTrenTrucTiep = CapTrenTrucTiep == null ? data[0].NhanSu : CapTrenTrucTiep;
                $('#iCapTrenTrucTiep').dxSelectBox({
                    dataSource: data,
                    valueExpr: 'NhanSu',
                    displayExpr: 'HoTen',
                    value: CapTrenTrucTiep,
                    width: '100%',
                    onValueChanged: function (e) {
                        CapTrenTrucTiep = e.value;
                    }
                });
            });
            $.get('/Portal/Data/GetCapTrenGianTiep').done(function (rs) {
                var data = JSON.parse(rs.ResponseData);
                CapTrenGianTiep = CapTrenGianTiep == null ? data[0].NhanSu : CapTrenGianTiep;
                $('#iCapTrenGianTiep').dxSelectBox({
                    dataSource: data,
                    valueExpr: 'NhanSu',
                    displayExpr: 'HoTen',
                    value: CapTrenGianTiep,
                    width: '100%',
                    onValueChanged: function (e) {
                        CapTrenGianTiep = e.value;
                    }
                });
            });

            $.get('/Portal/Data/Get_LoaiNghiPhep').done(function (rs) {
                var data = JSON.parse(rs.ResponseData);
                LoaiNghiPhep = LoaiNghiPhep == null ? data[0].MaLoaiNghiPhep : LoaiNghiPhep;
                $('#id_LoaiNghiPhep').dxSelectBox({
                    dataSource: data,
                    value: LoaiNghiPhep,
                    valueExpr: 'MaLoaiNghiPhep',
                    displayExpr: 'TenLoaiNghiPhep',
                    width: '100%',
                    onValueChanged: function (e) {
                        LoaiNghiPhep = e.value;
                    }
                });
            });
            $.get('/Portal/Data/Get_HinhThucNghiPhep').done(function (rs) {
                var data = JSON.parse(rs.ResponseData);
                HinhThucNghiPhep = HinhThucNghiPhep == null ? data[0].MaHinhThucNghiPhep : HinhThucNghiPhep;
                $('#id_HinhThucNghiPhep').dxSelectBox({
                    dataSource: data,
                    value: HinhThucNghiPhep,
                    valueExpr: 'MaHinhThucNghiPhep',
                    displayExpr: 'TenHinhThucNghiPhep',
                    width: '100%',
                    onValueChanged: function (e) {
                        HinhThucNghiPhep = e.value;
                    }
                });
            });

            $.get('/Portal/Data/Get_Nhom').done(function (rs) {
                var data = JSON.parse(rs.ResponseData);
                $('#id_Nhom').val(data[0].Nhom);
            });

        }
        var danhSachGrid = null;
        var isDanhSachGridInitializing = false;
        var isDanhSachGridReady = false;
        var isDanhSachReloading = false;
        var isDanhSachReloadPending = false;
        var danhSachReloadTimer = null;

        function SyncDanhSachGridDimensions() {
            window.requestAnimationFrame(function () {
                if (danhSachGrid != null) {
                    danhSachGrid.updateDimensions();
                }
            });
        }

        function QueueDanhSachReload() {
            isDanhSachReloadPending = true;

            if (danhSachGrid == null || !isDanhSachGridReady || isDanhSachReloading) {
                return;
            }

            clearTimeout(danhSachReloadTimer);
            danhSachReloadTimer = setTimeout(function () {
                if (danhSachGrid == null || isDanhSachReloading) {
                    return;
                }

                isDanhSachReloadPending = false;
                isDanhSachReloading = true;

                $.when(danhSachGrid.refresh()).always(function () {
                    isDanhSachReloading = false;
                    SyncDanhSachGridDimensions();

                    if (isDanhSachReloadPending) {
                        QueueDanhSachReload();
                    }
                });
            }, 150);
        ...
        function LoadDanhSach() {
            ...
            if (isDanhSachGridInitializing) {
                isDanhSachReloadPending = true;
                return;
            }

            isDanhSachGridInitializing = true;
            BSCSoft.getDataGridColums({ id: 'iDanhSach' }, function (c) {
                var columns = c;
                var check = false;
                $.each(c, function (i, v) {

                    if (v.dataField == 'isDuyet') {
                        v.alignment = 'center';
                        v.cellTemplate = function (container, options) {
                            var text = '';

                            switch (Number(options.data.isDuyet)) {
                                case -2:
                                    text = 'Nhân viên hủy đăng ký';
                                    break;
                                case 3:
                                    text = 'Chờ cấp trên gián tiếp';
                                    break;
                                case 1:
                                    text = 'Đã duyệt';
                                    break;
                    ...
                    if (v.columns != undefined && !check) {
                        v.fixed = true;
                        v.fixedPosition = "left";
                        check = true
                    }
                });
                $("#iDanhSach").dxDataGrid({
                    ...
                    onInitialized: function (e) {
                        danhSachGrid = e.component;
                    },
                    onContentReady: function (e) {
                        if (!isDanhSachGridReady) {
                            danhSachGrid = e.component;
                            isDanhSachGridReady = true;
                            SyncDanhSachGridDimensions();
                    ...
                    onCellPrepared: function (e) {
                        if (e.rowType === "data" && (e.column.dataField === "isDuyet")) {
                            var value = e.value;
                            if (value == 3) {
                                e.cellElement.css({ "background-color": "#20c997", 'color': '#fff' });
                            }
                            if (value == 2) {
                                e.cellElement.css({ "background-color": "#ecd9c6" });
                            }
                            if (value == -2) {
                                e.cellElement.css({ "background-color": "#f35A2A", 'color': '#fff' });
                            }
                            if (value == 1) {
                                e.cellElement.css({ "background-color": "#20c997", 'color': '#fff' });
                            }
                            if (value == 0) {
                                e.cellElement.css({ "background-color": "#ff4d4d", 'color': '#fff' });
                            }
                            if (value == -1) {
                    ...
                    onExporting: function (e) {
                       var exportInfo = BSCSoft.exportInfo('iDanhSach');
                       var workbook = new ExcelJS.Workbook();
                       var worksheet = workbook.addWorksheet('DATA', {
                           pageSetup: { paperSize: 9, orientation: 'landscape' }
                       });
                       var filename_new = ('').length > 0 ? '.xlsx' : exportInfo.fileName;
                       ...
                       DevExpress.excelExporter.exportDataGrid({
                           ...
                           customizeCell: function (options) {
                               ...
                               if (typeof excelCell.value === "string") {
                                   ...
                                   var doc = parser.parseFromString(excelCell.value, "text/html");
                                   excelCell.value = doc.body.textContent || "";
                               }

                               excelCell.font = { name: 'Arial', size: 10, color: { argb: '1F2937' } };
                               excelCell.alignment = BSCSoft.getExcelCellAlignment(gridCell);
                               if (gridCell.rowType === 'group') {
                                   excelCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'E1F0FF' } };
                                   excelCell.font = { name: 'Arial', color: { argb: '187DE4' }, bold: true, size: 11 };
                               } else if (gridCell.rowType === 'header') {
                                   excelCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'D9E2F3' } };
                                   excelCell.font = { name: 'Arial', color: { argb: '0A2A66' }, bold: true, size: 11 };
                                   excelCell.alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
                               } else if (gridCell.rowType === 'data') {
                                   if (gridCell.data && gridCell.rowIndex % 2 === 1) {
                                       excelCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'F8FAFC' } };
                                   }
                               } else if (gridCell.rowType === 'totalFooter' || gridCell.rowType === 'groupFooter') {
                                   excelCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'E2E8F0' } };
                                   excelCell.font = { name: 'Arial', bold: true, size: 10, color: { argb: '0F172A' } };
                               }
                               excelCell.border = {
                ...
                var headerSource = worksheet.getRow(1);
                headerSource.height = 24;
                headerSource.getCell(1).value = exportInfo.sourceText;
                headerSource.getCell(1).font = { name: 'Arial', size: 10, color: { argb: '5B6572' }, italic: true };
                headerSource.getCell(1).alignment = { vertical: 'middle', horizontal: 'left' };

                worksheet.mergeCells(2, 1, 2, 5);
                var headerTime = worksheet.getRow(2);
                headerTime.height = 20;
                headerTime.getCell(1).value = exportInfo.timeSpan;
                headerTime.getCell(1).font = { name: 'Arial', size: 10, color: { argb: '7A7F87' } };
                headerTime.getCell(1).alignment = { vertical: 'middle', horizontal: 'left' };

                var headerTable = worksheet.getRow(6);
                headerTable.height = 24;
                var headerTitle = worksheet.getRow(3);
                headerTitle.height = 34;
                worksheet.mergeCells(3, 1, 3, dataGridRange.to.column);
                headerTitle.getCell(1).value = ('').length > 0 ? '' : exportInfo.title;
                headerTitle.getCell(1).font = { name: 'Arial', size: 18, bold: true, color: { argb: '0F172A' } };
                headerTitle.getCell(1).alignment = { vertical: 'middle', horizontal: 'center' };
                BSCSoft.autoFitWorksheetColumns(worksheet, dataGridRange.from.row);
                       }).then(function () {
                           workbook.xlsx.writeBuffer().then(function (buffer) {
                               saveAs(new Blob([buffer], { type: 'application/octet-stream' }), filename_new);
                           });
                       });
                       e.cancel = true;
                   },
                ...
                danhSachGrid = $("#iDanhSach").dxDataGrid("instance");
                isDanhSachGridInitializing = false;
            });
        }

        function LoadDanhSach2() {
            $("#iDanhSach1").dxDataGrid({
                dataSource: {
                    store: dataSource
                },
                // paging: {
                ...
                "export": {
                    enabled: true,
                    fileName: 'Danh sách duyệt nghỉ phép',
                },
                columns: [
                    { dataField: 'MaNhanSu', caption: 'Mã nhân viên', width: 120, fixed: true, fixedPosition: "left", alignment: "center", },
                    { dataField: 'NguoiDangKy', caption: 'Tên người đăng ký', width: 180, fixed: true, fixedPosition: "left", alignment: "center", },
                    { dataField: 'Nhom', caption: 'Nhóm', width: 150 },
                    { dataField: 'ToDoi', caption: 'Tổ đội', width: 150 },
                    { dataField: 'TuNgay', caption: 'Từ ngày', width: 150, dataType: "date", format: "dd/MM/yyyy", alignment: "center", },
                    { dataField: 'DenNgay', caption: 'Đến ngày', width: 150, dataType: "date", format: "dd/MM/yyyy", alignment: "center", },
                    { dataField: 'SoNgayNghi', caption: 'Số ngày nghỉ', alignment: "center", },
                    { dataField: 'TenLoaiNghiPhep', caption: 'Loại nghỉ phép' },
                    { dataField: 'TenLoaiNghiPhep_ChiTiet', caption: 'Chi tiết các loại nghỉ' },
                    { dataField: 'TenHinhThucNghiPhep', caption: 'Hình thức nghỉ phép', alignment: "center", },
                    { dataField: 'LyDo', caption: 'Lý do nghỉ phép', width: 150 },
                    { dataField: 'TenCapTrenTrucTiep', caption: 'Cấp trên trực tiếp', width: 150},
                    { dataField: 'TenCapTrenGianTiep', caption: 'Cấp trên gián tiếp', width: 150 },
                    { dataField: 'LyDoTuChoi', caption: 'Lý do từ chối', width: 150 },
                     { dataField: 'LyDoHuyPhep', caption: 'Lý do hủy'},
                    {
                        dataField: 'isDuyet', caption: 'Trạng thái duyệt', fixed: true, fixedPosition: "right",  alignment: "center", cellTemplate: function (container, options) {
                            var data = options.data;
                            var html = '';
                            if (data.isDuyet == -2) {
                                html = 'Nhân viên hủy đăng ký';
                            }
                            if (data.isDuyet == 3) {
        ...
        <script type="text/javascript">
            ...
            function Duyet_Phep(TrangThai) {
                var selectedRows = $("#iDanhSach").dxDataGrid('instance').getSelectedRowsData();
                var nghiPhep_ID = [];
                var DuyetGianTiep_ID = [];

                $.each(selectedRows, function () {
                    nghiPhep_ID.push(this.NghiPhep);
                    DuyetGianTiep_ID.push(this.DuyetGianTiep);
                });

                if (nghiPhep_ID.length == 0) {
                ...
                if (culture == 'en') {
                    confirmMessage = 'You are approving <strong>' + nghiPhep_ID.length + '</strong> selected leave request(s). Please confirm to continue.';
                } else if (culture == 'jp') {
                    confirmMessage = '選択した休暇申請 <strong>' + nghiPhep_ID.length + '</strong> 件を承認します。続行するには確認してください。';
                } else {
                    confirmMessage = 'Bạn đang phê duyệt <strong>' + nghiPhep_ID.length + '</strong> đơn nghỉ phép đã chọn. Vui lòng xác nhận để tiếp tục.';
                }
                ...
                Swal.fire({
                    ...
                    $.post('/Portal/NghiPhep/Update_DuyetNghiPhep', {
                        NghiPhep_ID: nghiPhep_ID.join(','),
                        DuyetGianTiep_ID: DuyetGianTiep_ID.join(','),
                        LyDo: "",
                        TrangThai: TrangThai,
                        __RequestVerificationToken: $('input[name="__RequestVerificationToken"]').val()
                    }).done(function (rs) {
                        ...
                        DevExpress.ui.notify('Cập nhật thất bại', "error", 5000);
                    }).always(function () {
                        loadPanel.hide();
                    });
                });
            }
            function TuChoi_Phep(TrangThai) {
                var selectedRows = $("#iDanhSach").dxDataGrid('instance').getSelectedRowsData();
                var nghiPhep_ID = [];
                var DuyetGianTiep_ID = [];
                ...
                if (culture == 'en') {
                    confirmMessage = 'You are rejecting <strong>' + nghiPhep_ID.length + '</strong> selected leave request(s). Please enter a reason to continue.';
                } else if (culture == 'jp') {
                    confirmMessage = '選択した休暇申請 <strong>' + nghiPhep_ID.length + '</strong> 件を差し戻します。続行するには理由を入力してください。';
                } else {
                    confirmMessage = 'Bạn đang từ chối <strong>' + nghiPhep_ID.length + '</strong> đơn nghỉ phép đã chọn. Vui lòng nhập lý do để tiếp tục.';
                }
                ...
                Swal.fire({
                    ...
                    inputValidator: function (value) {
                        ...
                        if (lyDo.length == 0) {
                            return 'Vui lòng nhập lý do từ chối phê duyệt';
                        }

                        if (lyDo.length > 250) {
                            return 'Vui lòng không nhập quá 250 ký tự';
                        }
                    }
                }).then(function (result) {
                    if (!result.isConfirmed) {
